...
if($registros['status'] == 1){
    ...
    foreach($registros['data'] as $k => $v){
        if (array_key_exists($v['usr_id'], $users)) {
            $users[$v['usr_id']][$v['oprnme']] = $users[$v['usr_id']][$v['oprnme']] + $v['t_wrkday'];
            $users[$v['usr_id']]['Disponibilidade'] = $users[$v['usr_id']]['Disponibilidade'] - $v['t_wrkday'];
        }
    }
    $somaWorkDay = 0;
        ...
        foreach($date['interval'] as $dte => $dte_values){
            foreach($workDay['data'] as $k => $w_values){
                if($w_values['usr_id'] == $usr_id){
                    $users[$usr_id]['Disponibilidade'] = $users[$usr_id]['Disponibilidade'] + $w_values['wrkday'] * (1 - $regras->fadiga);
                    $somaWorkDay = $somaWorkDay + $w_values['wrkday'];
                }  
            }
        }
